...
// Inspired by the inspiring Fluent NHibernate

namespace EsentMapper.Mapping
{
	internal abstract class Member : IEquatable<Member>
	{
	...
	internal class PropertyMember : Member
	{
		...
		static MethodMember GetMember(MethodInfo method)
		{
			if (method == null)
				return null;
	...
	internal static class MemberExtensions
	{
		...
		public static IEnumerable<Member> GetInstanceMembers(this Type type)
		{
			...
			type.GetInstanceProperties().ToList().ForEach(x => members.Add(x));
			type.GetInstanceFields().ToList().ForEach(x => members.Add(x));
			type.GetInstanceMethods().ToList().ForEach(x => members.Add(x));

			if (type.BaseType != null && type.BaseType != typeof(object))
				type.BaseType.GetInstanceMembers().ToList().ForEach(x => members.Add(x));
